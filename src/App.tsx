        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 px-2">
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5 md:gap-2">
              {/* Pullbacks Filter */}
              <button
                onClick={() => setShowPullbacksOnly(prev => !prev)}
                className={`flex items-center gap-1 md:gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-xl border text-[10px] md:text-xs font-bold uppercase transition-all active:scale-95 cursor-pointer ${showPullbacksOnly
                    ? (isDarkMode ? 'bg-teal-500/20 text-teal-400 border-teal-500/40 shadow-[0_0_12px_rgba(20,184,166,0.25)]' : 'bg-teal-50 text-teal-700 border-teal-200 shadow-sm')
                    : (isDarkMode ? 'bg-slate-900 border-white/10 hover:border-teal-500/30 text-slate-400 hover:text-teal-300' : 'bg-white border-slate-200/60 hover:border-teal-500/30 text-slate-600 hover:text-teal-700 shadow-sm')
                  }`}
                title="Filter by Uptrend Pullbacks"
              >
                <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-[9px] font-black shrink-0 ${showPullbacksOnly ? 'bg-teal-400 text-teal-950' : 'bg-slate-500/20 text-slate-400'}`}>P</span>
                <span className="hidden sm:inline">Pullbacks</span>
                {uptrendPullbackCount > 0 && <span className="text-[9px] md:text-[10px] opacity-65 font-bold">({uptrendPullbackCount})</span>}
              </button>

              {/* Golden Stars Filter */}
              <button
                onClick={() => setShowGoldenStarsOnly(prev => !prev)}
                className={`flex items-center gap-1 md:gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-xl border text-[10px] md:text-xs font-bold uppercase transition-all active:scale-95 cursor-pointer ${showGoldenStarsOnly
                    ? (isDarkMode ? 'bg-amber-500/20 text-amber-400 border-amber-500/40 shadow-[0_0_12px_rgba(251,191,36,0.25)]' : 'bg-amber-50 text-amber-700 border-amber-200 shadow-sm')
                    : (isDarkMode ? 'bg-slate-900 border-white/10 hover:border-amber-500/30 text-slate-400 hover:text-amber-300' : 'bg-white border-slate-200/60 hover:border-amber-500/30 text-slate-600 hover:text-amber-700 shadow-sm')
                  }`}
                title="Filter by Golden Stars"
              >
                <Star className={`w-3.5 h-3.5 shrink-0 ${showGoldenStarsOnly ? 'fill-amber-400 text-amber-400' : 'text-slate-400'}`} />
